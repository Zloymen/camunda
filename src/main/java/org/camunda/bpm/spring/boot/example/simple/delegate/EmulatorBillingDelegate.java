...
public class EmulatorBillingDelegate implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) throws Exception {
        if(!execution.hasVariable("billingDate")) return;
        if(!execution.hasVariable("dataClient")) return;
        if(!execution.hasVariable("resultData")) return;
        if(!execution.hasVariable("id")) return;
